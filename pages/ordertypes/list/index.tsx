...
import { AppBar, Button, Card, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Pagination, TableCell, TableRow, Toolbar, Tooltip, Typography, Zoom, styled, tableCellClasses, Menu, Grid, ListItemButton, ListItemIcon } from '@mui/material';
...
export default function OrderTypesList(props: any) {
    ...
    const rows = (orderTypes || []).map(dataRow => {
            ...
            <>
                <div style={{ paddingBottom: 10 }}>
                    <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro' paddingBottom={1}
                        fontWeight={500} letterSpacing={'.2px'}>{OrderTypes.OrderTypeList}
                    </Typography>
                    <Divider sx={{ width: 500 }} />
                </div>
                <Row>
                    <Tooltip title={<Typography id='TypographyNeutro' letterSpacing={'.5px'} fontSize={'11px'}>Borrado masivo de 1 o mas tipos de orden seleccionados</Typography>} arrow placement="right">
                        <span>
                            <Button id='Button2' variant="contained" disabled={bulkList.length == 0} onClick={() => handleOpenDialog(2)}>{OrderTypes.BulkDelete}</Button>
                        </span>
                    </Tooltip>
                    <Spacer style={{ flex: 1 }} />
                    ...
                    <Button disabled={isLoadingToEdit} id='Button2' variant="contained" onClick={() => handleRouter('/ordertypes/register', 1)}>
                        {isLoadingToRegiser ? <CircularProgress color='inherit' size={25} thickness={4} /> : OrderTypes.Register}</Button>
                </Row>
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await axios.put(`/api/ordertypes`, { id, ErasedAt }).then(async (result) => {
                ...
                setIsLoadingDialog(false)
                enqueueSnackbar('El tipo de orden ha sido borrado correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
                ...
                handleClose()
                await mutate(`/api/ordertypes`, orderTypes, true);
            })
    ...
    const handleBulkDelete = async () => {
        try {
            ...
            const isBulk = true
            setIsLoadingDialog(true)
            await axios.put(`/api/ordertypes`, { bulkList, isBulk }).then(async (result) => {
                if (result.status != 200) {
                    setIsLoadingDialog(false)
                ...
                setIsLoadingDialog(false)
                enqueueSnackbar(bulkList.length == 1 ? 'El tipo de orden ha sido borrado correctamente' : 'Los tipos de orden han sido borrados correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
    ...
    const titlesArray = [OrderTypes.OrderType, OrderTypes.CreationDate]
        ...
        <>
            {
                openPDF
                    ?
                    <PDFLayout titles={titles} values={values} title={'Reporte de Tipos de Orden'} />
                    :
                    <>
                        <Layout>
                            <Dialog
                                ...
                                <AppBar sx={{ position: 'relative', backgroundColor: '#1c1c1c' }}>
                                    <Toolbar style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', alignContent: 'center' }}>
                                        <DialogTitle id="responsive-dialog-title" sx={{ letterSpacing: '1px', fontFamily: 'M PLUS 2', fontWeight: 400 }}>
                                            {dialogType == 1 ? 'Borrar tipo de orden' : 'Borrado Masivo'}
                                        </DialogTitle>
                                    </Toolbar>
                                </AppBar>
                                        ...
                                        isLoadingDialog ? <CircularProgress size={50} color='inherit' /> :
                                            <>
                                                <Typography id='TypographyNeutro' fontWeight={500} textAlign={'left'}>
                                                    {dialogType == 1 ? '¿Esta seguro que desea borrar este tipo de orden?' :
                                                        (bulkList.length == 1 ? '¿Esta seguro que desea eliminar este tipo de orden?' :
                                                            '¿Esta seguro que desea eliminar estos tipos de orden?')}
                                                </Typography>
                            ...
                            <DataGridPro
                                ...
                                {
                                    field: 'ordertype',
                                    headerName: OrderTypes.OrderType,
                                    minWidth: 400,
                                    editable: false,
                                    align: 'center',
                                    headerAlign: 'center',
                                ...
                                {
                                    field: 'createdAt',
                                    headerName: OrderTypes.CreationDate,
                                    width: 300,
                                    editable: false,
                                    align: 'center',
                                    headerAlign: 'center',
                                ...
                                },
                                {
                                    ...
                                    getActions: ({ id }) => [
                                        <>
                                            {
                                                <>
                                                    <GridActionsCellItem disabled={isLoadingToRegiser || isLoadingDialog} icon={<Tooltip arrow placement='left' title={<Typography id='TypographyNeutro' fontSize={'12px'}>{OrderTypes.Edit}</Typography>}>{id == idEdit && isLoadingToEdit ? <CircularProgress color="inherit" thickness={5} size={20} /> : <EditIcon />}</Tooltip>} key={Number(id) + Math.random()} onClick={() => { setIdEdit(Number(id)), handleRouter(`/ordertypes/edit/${id}`, 2) }} sx={{ ':hover': { color: 'green' }, color: "#393939", p: 1 }} label="Edit" />
                                                </>
                                            }
                                            {
                                                <>
                                                    <GridActionsCellItem disabled={isLoadingToRegiser || isLoadingDialog} icon={<Tooltip arrow placement='right' title={<Typography id='TypographyNeutro' fontSize={'12px'}>{OrderTypes.Delete}</Typography>}><DeleteIcon /></Tooltip>} key={Number(id) + Math.random()} onClick={() => { handleOpenDialog(1), setIdDelete(Number(id)) }} sx={{ ':hover': { color: 'red' }, color: "#393939", p: 1 }} label="Delete" />
                                                </>
                                            }
