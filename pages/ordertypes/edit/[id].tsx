...
export default function OrderTypesRegister(props: any) {
    ...
    const { orderTypes, isLoadingOrderTypes } = useOrderTypes(`/ordertypes?id=${router.query.id}`)

    const orderType = (orderTypes || []).map(dataRow => {
        return {
            id: dataRow.Id,
    ...
    const onRegisterForm = async (data: IOrderTypes) => {
        ...
        try {
            ...
            const id = orderType[0]?.id

            await axios.put(`/api/ordertypes`, { id, OrderType }).then((result) => {

                if (result.status != 200) {
                    return
                }
                enqueueSnackbar('El tipo de orden ha sido actualizado correctamente', {
                    variant: 'success',
                ...
                isLoadingOrderTypes
                    ...
                    <Form onSubmit={handleSubmit(onRegisterForm)} style={{ display: 'grid', placeContent: 'center', placeItems: 'center' }}>
                            ...
                            <Grid container spacing={4} padding={2}>
                                <Grid item xs={12}>
                                    ...
                                    <TextField style={{ width: 700 }} id='OrderType' fullWidth sx={style} variant='filled'
                                        ...
                                        defaultValue={orderType[0]?.OrderType}
                        ...
                        <Button disabled={isSubmitting} type="submit" id="Button2" variant="contained" fullWidth sx={{ margin: 1 }}>
                            {isSubmitting ? <CircularProgress color='inherit' size={25} thickness={4} /> : OrderTypes.Edit}
                        </Button>
                    </Form>
            }
