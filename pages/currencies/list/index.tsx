...
export default function CurrenciesList(props: any) {
    ...
    const rows = (currencies || []).map(dataRow => {
            ...
            <>
                <div style={{ paddingBottom: 10 }}>
                    <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro' paddingBottom={1}
                        fontWeight={500} letterSpacing={'.2px'}>{Currencies.CurrencyList}
                    </Typography>
                    <Divider sx={{ width: 500 }} />
                </div>
                <Row>
                    <Tooltip title={<Typography id='TypographyNeutro' letterSpacing={'.5px'} fontSize={'11px'}>Borrado masivo de 1 o mas tipos de moneda seleccionados</Typography>} arrow placement="right">
                        <span>
                            <Button id='Button2' variant="contained" disabled={bulkList.length == 0} onClick={() => handleOpenDialog(2)}>{Currencies.BulkDelete}</Button>
                        </span>
                    </Tooltip>
                    <Spacer style={{ flex: 1 }} />
                    ...
                    <Button disabled={isLoadingToEdit} id='Button2' variant="contained" onClick={() => handleRouter('/currencies/register', 1)}>
                        {isLoadingToRegiser ? <CircularProgress color='inherit' size={25} thickness={4} /> : Currencies.Register}</Button>
                </Row>
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await axios.put(`/api/ordertypes`, { id, ErasedAt }).then(async (result) => {
                ...
                setIsLoadingDialog(false)
                enqueueSnackbar('El tipo de moneda ha sido borrado correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
                ...
                handleClose()
                await mutate(`/api/currencies`, currencies, true);
            })
    ...
    const handleBulkDelete = async () => {
        try {
            ...
            const isBulk = true
            setIsLoadingDialog(true)
            await axios.put(`/api/currencies`, { bulkList, isBulk }).then(async (result) => {
                if (result.status != 200) {
                    setIsLoadingDialog(false)
                ...
                setIsLoadingDialog(false)
                enqueueSnackbar(bulkList.length == 1 ? 'El tipo de moneda ha sido borrado correctamente' : 'Los tipos de orden han sido borrados correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
    ...
    const titlesArray = [Currencies.Currency, Currencies.CreationDate]
            ...
            if (index == indexSplit) {
                ...
                return (
                    <>
                        ...
                        >
                            <StyledTableCell component="th" scope="row">{item.currency}</StyledTableCell>
        ...
        <>
            {
                openPDF
                    ?
                    <PDFLayout titles={titles} values={values} title={'Reporte de Tipos de Moneda'} />
                    :
                    <>
                        <Layout>
                            <Dialog
                                ...
                                <AppBar sx={{ position: 'relative', backgroundColor: '#1c1c1c' }}>
                                    <Toolbar style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', alignContent: 'center' }}>
                                        <DialogTitle id="responsive-dialog-title" sx={{ letterSpacing: '1px', fontFamily: 'M PLUS 2', fontWeight: 400 }}>
                                            {dialogType == 1 ? 'Borrar tipo de moneda' : 'Borrado Masivo'}
                                        </DialogTitle>
                                    </Toolbar>
                                </AppBar>
                                        ...
                                        isLoadingDialog ? <CircularProgress size={50} color='inherit' /> :
                                            <>
                                                <Typography id='TypographyNeutro' fontWeight={500} textAlign={'left'}>
                                                    {dialogType == 1 ? '¿Esta seguro que desea borrar este tipo de moneda?' :
                                                        (bulkList.length == 1 ? '¿Esta seguro que desea eliminar este tipo de moneda?' :
                                                            '¿Esta seguro que desea eliminar estos tipos de moneda?')}
                                                </Typography>
                            ...
                            <DataGridPro
                                ...
                                {
                                    field: 'currency',
                                    headerName: Currencies.Currency,
                                    minWidth: 400,
                                    editable: false,
                                    align: 'center',
                                    headerAlign: 'center',
                                ...
                                {
                                    field: 'createdAt',
                                    headerName: Currencies.CreationDate,
                                    width: 300,
                                    editable: false,
                                    align: 'center',
                                    headerAlign: 'center',
                                ...
                                },
                                {
                                    ...
                                    getActions: ({ id }) => [
                                        <>
                                            {
                                                <>
                                                    <GridActionsCellItem disabled={isLoadingToRegiser || isLoadingDialog} icon={<Tooltip arrow placement='left' title={<Typography id='TypographyNeutro' fontSize={'12px'}>{Currencies.Edit}</Typography>}>{id == idEdit && isLoadingToEdit ? <CircularProgress color="inherit" thickness={5} size={20} /> : <EditIcon />}</Tooltip>} key={Number(id) + Math.random()} onClick={() => { setIdEdit(Number(id)), handleRouter(`/currencies/edit/${id}`, 2) }} sx={{ ':hover': { color: 'green' }, color: "#393939", p: 1 }} label="Edit" />
                                                </>
                                            }
                                            {
                                                <>
                                                    <GridActionsCellItem disabled={isLoadingToRegiser || isLoadingDialog} icon={<Tooltip arrow placement='right' title={<Typography id='TypographyNeutro' fontSize={'12px'}>{Currencies.Delete}</Typography>}><DeleteIcon /></Tooltip>} key={Number(id) + Math.random()} onClick={() => { handleOpenDialog(1), setIdDelete(Number(id)) }} sx={{ ':hover': { color: 'red' }, color: "#393939", p: 1 }} label="Delete" />
                                                </>
                                            }
