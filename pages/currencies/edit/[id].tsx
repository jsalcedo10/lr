...
export default function CurrenciesEdit(props: any) {
    ...
    const { currencies, isLoadingCurrency } = useCurrency(`/currencies?id=${router.query.id}`)

    const currency = (currencies || []).map(dataRow => {
        return {
            id: dataRow.Id,
    ...
    const onRegisterForm = async (data: ICurrencies) => {
        ...
        try {
            ...
            const id = currency[0]?.id

            await axios.put(`/api/currencies`, { id, Currency }).then((result) => {
        ...
        <EntityLayout>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignContent: 'center', alignItems: 'end' }}>
                <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro'
                    fontWeight={500} letterSpacing={'.2px'}>{Currencies.EditCurrency}</Typography>
                <Spacer css={{ flex: 1 }} />
                ...
                isLoadingCurrency
                    ...
                    <Form onSubmit={handleSubmit(onRegisterForm)} style={{ display: 'grid', placeContent: 'center', placeItems: 'center' }}>
                            ...
                            <Grid container spacing={4} padding={2}>
                                <Grid item xs={12}>
                                    ...
                                    <TextField style={{ width: 700 }} id='Currency' fullWidth sx={style} variant='filled'
                                        ...
                                        defaultValue={currency[0]?.Currency}
                        ...
                        <Button disabled={isSubmitting} type="submit" id="Button2" variant="contained" fullWidth sx={{ margin: 1 }}>
                            {isSubmitting ? <CircularProgress color='inherit' size={25} thickness={4} /> : Currencies.Edit}
                        </Button>
                    </Form>
            }
