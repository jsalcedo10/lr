...
export default function ProductEdit(props: any) {
    ...
    const { products, isLoadingProduct } = useProducts(`/products?id=${router.query.id}`)

    const product = (products || []).map(dataRow => {
    ...
    const onRegisterForm = async (data: IProduct) => {
        ...
        try {
            ...
            if (router.query.id != undefined || router.query.id == null) {
                if (router.query.id?.toString() != product[0]?.id.toString()) {
                    enqueueSnackbar('La url correspondiente fue modificada, favor de volverlo a intentar.', {
                        variant: 'error',
            ...
            await axios.put(`/api/products`, { id, Product, Description, Type, Unit, Octane, IEPS }).then((result) => {

                if (result.status != 200) {
                    return
                }
                enqueueSnackbar('El producto ha sido actualizado correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
        ...
        <EntityLayout>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignContent: 'center', alignItems: 'end' }}>
                <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro'
                    fontWeight={500} letterSpacing={'.2px'}>{Products.EditProduct}</Typography>
                <Spacer css={{ flex: 1 }} />
                ...
                isLoadingProduct || product.length == 0
                    ...
                    <Form onSubmit={handleSubmit(onRegisterForm)} style={{ display: 'grid', placeContent: 'center', placeItems: 'center' }}>
                            ...
                            <Grid container spacing={4} padding={2}>
                                <Grid item xs={12} lg={6}>
                                    ...
                                    <TextField id='Product' fullWidth sx={style} variant='filled'
                                        ...
                                        defaultValue={product[0]?.Product}
                        ...
                        <Button disabled={isSubmitting} type="submit" id="Button2" variant="contained" fullWidth sx={{ margin: 1 }}>
                            {isSubmitting ? <CircularProgress color='inherit' size={25} thickness={4} /> : Products.Edit}
                        </Button>
                    </Form>
            }
        </EntityLayout>
