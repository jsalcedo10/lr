...
import { DataGridPro, GRID_CHECKBOX_SELECTION_COL_DEF, GridActionsCell, GridActionsCellItem, GridCsvExportOptions, GridCsvGetRowsToExportParams, gridPageCountSelector, gridVisibleSortedRowIdsSelector, useGridApiContext, useGridSelector } from "@mui/x-data-grid-pro";
import { AppBar, Button, Card, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Pagination, Popper, TableCell, TableRow, Toolbar, Tooltip, Typography, Zoom, styled, tableCellClasses, Menu, Grid, ListItemButton } from '@mui/material';
...
export default function ProductList(props: any) {
    ...
    const rows = (products || []).map(dataRow => {
            ...
            <>
                <div style={{ paddingBottom: 10 }}>
                    <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro' paddingBottom={1}
                        fontWeight={500} letterSpacing={'.2px'}>{Products.ProductList}
                    </Typography>
                    <Divider sx={{ width: 500 }} />
                </div>
                <Row>
                    <Tooltip title={<Typography id='TypographyNeutro' letterSpacing={'.5px'} fontSize={'11px'}>Borrado masivo de 1 o mas productos seleccionados</Typography>} arrow placement="right">
                        <span>
                            <Button id='Button2' variant="contained" disabled={bulkList.length == 0} onClick={() => handleOpenDialog(2)}>{Products.BulkDelete}</Button>
                        </span>
                    </Tooltip>
                    <Spacer style={{ flex: 1 }} />
                    ...
                    <Button disabled={isLoadingToEdit} id='Button2' variant="contained" onClick={() => handleRouter('/products/register', 1)}>
                        {isLoadingToRegiser ? <CircularProgress color='inherit' size={25} thickness={4} /> : Products.Register}</Button>
                </Row>
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await axios.put(`/api/products`, { id, ErasedAt }).then(async (result) => {
                ...
                setIsLoadingDialog(false)
                enqueueSnackbar('El producto ha sido borrado correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
                ...
                handleClose()
                await mutate(`/api/products`, products, true);
            })
    ...
    const handleBulkDelete = async () => {
        try {
            ...
            const isBulk = true
            setIsLoadingDialog(true)
            await axios.put(`/api/products`, { bulkList, isBulk }).then(async (result) => {
                if (result.status != 200) {
                    setIsLoadingDialog(false)
                ...
                setIsLoadingDialog(false)
                enqueueSnackbar(bulkList.length == 1 ? 'El producto ha sido borrado correctamente' : 'Los productos han sido borrados correctamente', {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
    ...
    const titlesArray = [Products.Product, Products.Description, Products.Type, Products.Unit, Products.Octane,
    Products.IEPS, Products.CreationDate]
                ...
                return (
                    <>
                        ...
                        >
                            ...
                            <StyledTableCell component="th" scope="row">{item.octane}</StyledTableCell>
                            <StyledTableCell component="th" scope="row">{item.ieps}</StyledTableCell>
        ...
        <>
            {
                openPDF
                    ?
                    <PDFLayout titles={titles} values={values} title={'Reporte de Productos'} />
                    :
                    <><Layout>
                        <Dialog
                            className="fadeIn"
                            ...
                            <AppBar sx={{ position: 'relative', backgroundColor: '#1c1c1c' }}>
                                <Toolbar style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', alignContent: 'center' }}>
                                    <DialogTitle id="responsive-dialog-title" sx={{ letterSpacing: '1px', fontFamily: 'M PLUS 2', fontWeight: 400 }}>
                                        {dialogType == 1 ? 'Borrar producto' : 'Borrado Masivo'}
                                    </DialogTitle>
                                </Toolbar>
                            </AppBar>
                                    ...
                                    isLoadingDialog ? <CircularProgress size={50} color='inherit' /> :
                                        <>
                                            <Typography id='TypographyNeutro' fontWeight={500} textAlign={'left'}>
                                                {dialogType == 1 ? '¿Esta seguro que desea borrar este producto?' :
                                                    (bulkList.length == 1 ? '¿Esta seguro que desea eliminar este producto?' :
                                                        '¿Esta seguro que desea eliminar estos productos?')}
                                            </Typography>
                        ...
                        <DataGridPro
                            ...
                            {
                                field: 'createdAt',
                                headerName: Products.CreationDate,
                                width: 300,
                                editable: false,
                                align: 'center',
                                headerAlign: 'center',
