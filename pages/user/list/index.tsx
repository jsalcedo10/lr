...
import { DataGridPro, GRID_CHECKBOX_SELECTION_COL_DEF, GridActionsCell, GridActionsCellItem, gridPageCountSelector, useGridApiContext, useGridSelector, GridCsvGetRowsToExportParams, gridVisibleSortedRowIdsSelector, GridCsvExportOptions } from '@mui/x-data-grid-pro';
...
export default function UsersList(props: any) {
  ...
  const { user, isLoadingUser } = useUsers(`/user/register`)
  ...
  const rows = (user || []).map(dataRow => {

    return {
      id: dataRow.Id,
      username: dataRow.UserName,
      email: dataRow.Email,
      isadmin: dataRow.IsAdmin == 1 ? 'Si' : 'No',
      ...
      <>
        <div style={{ paddingBottom: 10 }}>
          <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro' paddingBottom={1}
            fontWeight={500} letterSpacing={'.2px'}>{Users.UserList}
          </Typography>
          <Divider sx={{ width: 500 }} />
        </div>
        <Row>
          <Tooltip title={<Typography id='TypographyNeutro' letterSpacing={'.5px'} fontSize={'11px'}>Borrado masivo de 1 o mas usuarios seleccionados</Typography>} arrow placement="right">
            <span>
              <Button id='Button2' variant="contained" disabled={bulkList.length == 0} onClick={() => handleOpenDialog(2)}>{Users.BulkDelete}</Button>
            </span>
          </Tooltip>
          <Spacer style={{ flex: 1 }} />
          ...
          <Button disabled={isLoadingToEdit} id='Button2' variant="contained" onClick={() => handleRouter('/user/register', 1)}>
            {isLoadingToRegiser ? <CircularProgress color='inherit' size={25} thickness={4} /> : Users.Register}</Button>
        </Row>
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.put(`/api/user/register`, { id, ErasedAt }).then(async (result) => {
        ...
        setIsLoadingDialog(false)
        enqueueSnackbar('El usuario ha sido borrado correctamente', {
          variant: 'success',
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: 'bottom',
        ...
        handleClose()
        await mutate(`/api/user/register`, user, true);
      })
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      const isBulk = true
      setIsLoadingDialog(true)
      await axios.put(`/api/user/register`, { bulkList, isBulk }).then(async (result) => {
        if (result.status != 200) {
          setIsLoadingDialog(false)
        ...
        setIsLoadingDialog(false)
        enqueueSnackbar(bulkList.length == 1 ? 'El usuario ha sido borrado correctamente' : 'Los usuarios han sido borrados correctamente', {
          variant: 'success',
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: 'bottom',
  ...
  const titlesArray = [Users.UserName, Users.Email, Users.IsAdmin, Users.CreationDate]
      ...
      if (index == indexSplit) {
        ...
        return (
          <>
            ...
            >
              <StyledTableCell component="th" scope="row">{item.username}</StyledTableCell>
              <StyledTableCell component="th" scope="row">{item.email}</StyledTableCell>
              <StyledTableCell component="th" scope="row">{item.isadmin}</StyledTableCell>
    ...
    <>
      {
        openPDF
          ?
          <PDFLayout titles={titles} values={values} title={'Reporte de Usuarios'} />
          :
          <>
            <Layout>
              <Dialog
                ...
                <AppBar sx={{ position: 'relative', backgroundColor: '#1c1c1c' }}>
                  <Toolbar style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', alignContent: 'center' }}>
                    <DialogTitle id="responsive-dialog-title" sx={{ letterSpacing: '1px', fontFamily: 'M PLUS 2', fontWeight: 400 }}>
                      {dialogType == 1 ? 'Borrar usuario' : 'Borrado Masivo'}
                    </DialogTitle>
                  </Toolbar>
                </AppBar>
                    ...
                    isLoadingDialog ? <CircularProgress size={50} color='inherit' /> :
                      <>
                        <Typography id='TypographyNeutro' fontWeight={500} textAlign={'left'}>
                          {dialogType == 1 ? '¿Esta seguro que desea borrar este usuario?' :
                            (bulkList.length == 1 ? '¿Esta seguro que desea eliminar este usuario?' :
                              '¿Esta seguro que desea eliminar estos usuarios?')}
                        </Typography>
                ...
                },
                {
                  ...
                  getActions: ({ id }) => [
                    <>
                      {
                        <>
                          <GridActionsCellItem disabled={isLoadingToRegiser || isLoadingDialog} icon={<Tooltip arrow placement='left' title={<Typography id='TypographyNeutro' fontSize={'12px'}>{Users.Edit}</Typography>}>{id == idEdit && isLoadingToEdit ? <CircularProgress color="inherit" thickness={5} size={20} /> : <EditIcon />}</Tooltip>} key={Number(id) + Math.random()} onClick={() => { setIdEdit(Number(id)), handleRouter(`/user/edit/${id}`, 2) }} sx={{ ':hover': { color: 'green' }, color: "#393939", p: 1 }} label="Edit" />
                        </>
                      }
                      {
                        <>
                          <GridActionsCellItem disabled={isLoadingToRegiser || isLoadingDialog} icon={<Tooltip arrow placement='right' title={<Typography id='TypographyNeutro' fontSize={'12px'}>{Users.Delete}</Typography>}><DeleteIcon /></Tooltip>} key={Number(id) + Math.random()} onClick={() => { handleOpenDialog(1), setIdDelete(Number(id)) }} sx={{ ':hover': { color: 'red' }, color: "#393939", p: 1 }} label="Delete" />
                        </>
                      }
