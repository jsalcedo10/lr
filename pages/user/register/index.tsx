...
export default function UsersRegister(props: any) {
        ...
        <EntityLayout>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignContent: 'center', alignItems: 'end' }}>
                <Typography variant={windowDimensions.width < 1900 ? 'h5' : 'h4'} id='TypographyNeutro'
                    fontWeight={500} letterSpacing={'.2px'}>{Users.RegisterUser}</Typography>
                <Spacer css={{ flex: 1 }} />
                ...
                <Card id='Card' >
                    <Grid container spacing={4} padding={2}>
                        ...
                        <Grid item xs={12} >
                            ...
                            <FormControlLabel control={<Checkbox checked={isAdmin} id="IsAdmin" {...register('IsAdmin', { required: false, onChange: handleIsAdmin })} />} label={<Typography id='Typography'>{'Administrador'}</Typography>} />
                        </Grid>
                    </Grid>
                </Card>
                <Button disabled={isSubmitting} type="submit" id="Button2" variant="contained" fullWidth sx={{ margin: 1 }}>
                    {isSubmitting ? <CircularProgress color='inherit' size={25} thickness={4} /> : Users.Register}
                </Button>
            </Form>
